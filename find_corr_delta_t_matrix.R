...
find_corr_delta_t_matrix <- function(w_wsd_data) {
  #get data
  vec_idx_code = get_industry_index_list()
  ...
  mat_corr = matrix(nrow = (length(w_wsd_data$Data) - 1), ncol = (length(w_wsd_data$Data) - 1))
  cat(c('Dim of the correlation matrix:', dim(mat_corr), '\n'))
  
  for (i in c(1 : length(mat_corr[1, ]))) {
    for (j in c(1 : length(mat_corr[, 1]))) {
      
      if (is.null(w_wsd_data$Data[i + 1]) || is.null(w_wsd_data$Data[j + 1])) {
